...
forked from: https://github.com/Trax-air/swagger-parser/blob/master/swagger_parser/swagger_parser.py
...
# dependency comes with bravado-core
...
class SwaggerParser(object):
    ...
    def __init__(self, swagger_path=None, swagger_dict=None, use_example=True, mode='sample-max'):
        """Run parsing from either a file or a dict.

        :param swagger_path: path of the swagger file.
        :param swagger_dict: swagger dict.
        :param use_example: Define if we use the example from the YAML when we
        ...
        self.build_definitions_example(mode)
        self.paths = {}
        self.operation = {}
    ...
    def build_definitions_example(self, mode='sample-max'):
        """Parse all definitions in the swagger specification."""
        for def_name, def_spec in self.specification.get('definitions', {}).items():
            self.build_one_definition_example(def_name, mode)

    def build_one_definition_example(self, def_name, mode='sample-max'):
        ...
        if def_spec.get('type') == 'array' and 'items' in def_spec:
            item = self.get_example_from_prop_spec(def_spec['items'], mode)
            self.definitions_example[def_name] = [item]
            return True

        if 'properties' not in def_spec:
            self.definitions_example[def_name] = self.get_example_from_prop_spec(def_spec, mode)
            return True
    ...
    def get_example_from_prop_spec(self, prop_spec, mode='sample-max'):
