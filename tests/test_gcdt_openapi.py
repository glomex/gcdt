...
forked from: https://github.com/Trax-air/swagger-parser/blob/master/tests/test_swagger_parser.py
...
from gcdt.gcdt_openapi import SwaggerParser
from . import here


@pytest.fixture
def swagger_parser():
    return SwaggerParser(here('resources/gcdt_openapi/swagger.yaml'))


@pytest.fixture
def inline_parser():
    return SwaggerParser(here('resources/gcdt_openapi/inline.yaml'))


@pytest.fixture(scope="module",
                params=[here('resources/gcdt_openapi/no_properties.yaml'),
                        here('resources/gcdt_openapi/object_no_schema.yaml'),
                        here('resources/gcdt_openapi/allof.yaml'),
                        here('resources/gcdt_openapi/array_ref_simple.yaml'),
                        here('resources/gcdt_openapi/null_type.yaml'),
                        here('resources/gcdt_openapi/array_items_list.yaml'),
                        here('resources/gcdt_openapi/type_list.yaml'),
                        ])
...
def inline_example():
    return {
        '3bd818fb7d55daf2fb8bf3354c061f9ba7f8cece39b30bdcb7e05551053ec2e8': (
        '/test', 'post', None)
    }
...
def swagger_array_parser():
    return SwaggerParser(here('resources/gcdt_openapi/swagger_arrays.yaml'))
...
def test_validate_petstore_swagger_json():
    complete_json = requests.get(
        "http://petstore.swagger.io/v2/swagger.json").json()
...
def test_build_definitions_example(swagger_parser, pet_definition_example):
    ...
    swagger_parser.specification['definitions']['Pet']['properties'][
        'category']['$ref'] = '#/definitions/Error'
...
def test_get_paths_data(swagger_parser, post_put_path_data, get_path_data):
    ...
    post_pet_id = \
    swagger_parser.paths['/v2/pets/{petId}']['post']['parameters']['petId']
    delete_pet_id = \
    swagger_parser.paths['/v2/pets/{petId}']['delete']['parameters']['petId']
...
def test_get_definition_name_from_ref(swagger_parser):
    assert swagger_parser.get_definition_name_from_ref(
        '#/definitions/Pet') == 'Pet'


def test_get_path_spec(swagger_parser):
    assert swagger_parser.get_path_spec('/v2/pets')[0] == '/v2/pets'
    assert swagger_parser.get_path_spec('/v2/users/createWithList')[
               0] == '/v2/users/createWithList'
    assert swagger_parser.get_path_spec('/v2/stores/order/1253')[
               0] == '/v2/stores/order/{orderId}'
...
# for validation we use bravado-core so we disable all code related to validation
'''
...
def test_validate_request(swagger_parser, pet_definition_example):
    ...
    assert not swagger_parser.validate_request('/v2/pets/findByTags', 'get',
                                               query={'tags': 'string'})
    ...
    # valid query
    assert swagger_parser.validate_request('/v2/pets/findByTags', 'get',
                                           query={'tags': ['string']})


def test_get_request_data(swagger_parser, pet_definition_example):
    assert swagger_parser.get_request_data('error', 'get') == {400: ''}
    assert swagger_parser.get_request_data('/v2/pets/123', 'get') == {
        200: pet_definition_example, 400: '', 404: ''}
...
def test_get_send_request_correct_body(swagger_parser, pet_definition_example):
    assert swagger_parser.get_send_request_correct_body('/v2/pets',
                                                        'post') == pet_definition_example
    assert swagger_parser.get_send_request_correct_body('/v2/pets/findByStatus',
                                                        'get') is None
    assert swagger_parser.get_send_request_correct_body('/v2/users/username',
                                                        'put') == 'string'
'''


# handling means validation
'''
def test_simple_additional_property_handling(swagger_parser):
    # value of type = int
    additional_properties_1 = {'any_prop2': 42, 'any_prop1': 42}
    valid_response_1 = {'aa': 3, 'ssssssss': 1, 'Not available': 6, 'xyz': 1,
                        'yyy': 2}
    ...
    valid_response_2 = {'aa': '3', 'ssssssss': 'one', 'Not available': '6',
                        555: 'yes', 'yyy': '$$$'}
    bad_response_2 = {'a': '1', 'b': 222, 35: '23', 'c': False}
    assert swagger_parser.validate_additional_properties(
        additional_properties_2, valid_response_2)
    assert not swagger_parser.validate_additional_properties(
        additional_properties_2, bad_response_2)
...
def test_complex_additional_property_handling(swagger_parser):
    ...
    assert swagger_parser.validate_additional_properties(
        additional_properties_3, valid_response_3)
    assert not swagger_parser.validate_additional_properties(
...
def test_referenced_additional_property_handling(swagger_parser):
    ...
    assert swagger_parser.validate_additional_properties(additional_properties,
                                                         valid_response)
    assert not swagger_parser.validate_additional_properties(
        additional_properties, bad_response)
...
def test_list_additional_property_handling(swagger_parser):
    ...
    assert swagger_parser.validate_additional_properties(
        additional_properties_4, valid_response_4)
    assert not swagger_parser.validate_additional_properties(
        additional_properties_4, bad_response_4)
'''
